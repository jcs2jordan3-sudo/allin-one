              ...
              return (
                ...
                >
                  ...
                  <span className={`ml-auto font-bold num ${top3 ? 'text-gold text-lg' : 'text-ink'}`}>
                    {fmtNum(m.rp)}<span className="text-[15px] font-semibold text-mut ml-0.5">RP</span>
                  </span>
                </div>
              )
            })}
            ...
            {history.map((s) => (
              <Card key={s.id} className="p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[15px] text-mut num">{s.closedAt ? fmtDateTime(s.closedAt) : ''} 마감</span>
                </div>
                <div className="space-y-1.5">
                  {s.results?.slice(0, 5).map((r) => (
                    <div key={r.memberId} className="flex items-center gap-3 text-sm">
                      ...
                      {r.paid ? <span className="num text-gold text-[16px]">+{fmtNum(r.paid)}P</span> : null}
                    </div>
                  ))}
                </div>
              </Card>
