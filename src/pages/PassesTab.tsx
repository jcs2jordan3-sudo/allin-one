        ...
        <div className="space-y-4">
          <Card className="p-5">
            <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
              <h3 className="font-bold">
                영업일 집계 <span className="text-[12px] text-mut font-normal ml-1 num">마지막 초기화 {fmtDateTime(st.bizResetAt)}</span>
              </h3>
          ...
          <Card className="p-5">
            <h3 className="font-bold mb-4">
              당월 집계 <span className="text-[12px] text-mut font-normal ml-1">※ 1일 ~ 현재</span>
            </h3>
            ...
            {pageRows.map((p) => {
              ...
              return (
                <Card key={p.id} className={`px-4 py-3 flex flex-wrap items-center gap-3 ${vs === 'used' || vs === 'revoked' ? 'opacity-60' : ''}`}>
                  ...
                  <span className="text-[12px] text-mut num">발급 {fmtDateTime(p.issuedAt)}</span>
                  <span className={`text-[12px] num ${vs === 'expired' ? 'text-rose font-semibold' : 'text-mut'}`}>
                    {vs === 'used' && p.usedAt
            ...
            {logRows.map((l) => {
              ...
              return (
                <Card key={l.id} className="px-4 py-2.5 flex flex-wrap items-center gap-3 text-sm">
                  ...
                  {l.detail && <span className="text-mut text-[13px]">{l.detail}</span>}
  ...
  return (
    <Modal open onClose={onClose} title="이용권 발급">
      <div className="space-y-4">
        ...
        {selected && (
          <p className="text-[12px] text-mut -mt-2">유효기간 발급일로부터 <b className="text-ink">{selected.validDays}일</b></p>
        )}
        <Field label="회원 검색">
  ...
  return (
    <Modal open onClose={onClose} title="이용권 유형 관리" wide>
      <div className="space-y-2">
        ...
        {error && <div className="text-sm text-rose">{error}</div>}
        <p className="text-[12px] text-mut pt-2">
