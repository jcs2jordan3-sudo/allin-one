  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        <SectionTitle
          right={
            <label className="flex items-center gap-2 text-[13px] text-mut cursor-pointer">
              <input
                type="checkbox"
                checked={includeLeft}
      ...
      <section>
        ...
        >
          포인트 거래내역 <span className="text-[12px] text-mut font-normal ml-1">증감은 지점 잔액 기준</span>
        </SectionTitle>
        {pageRows.length === 0 ? (
      ...
      <Modal open={qrOpen} onClose={() => setQrOpen(false)} title="매장 가입 QR">
        <div className="flex flex-col items-center gap-4 py-2">
          ...
          <p className="text-[13px] text-mut text-center">
    ...
    <Card className="px-4 py-3">
      <button onClick={onToggle} className="w-full flex flex-wrap items-center gap-x-3 gap-y-1 text-left">
        ...
        <span className="ml-auto flex items-center gap-3">
          <span className="text-[13px] text-mut num">잔여 {fmtNum(l.storeBalanceAfter)}{CURRENCY_UNIT[l.currency]}</span>
          <span className={`text-mut text-xs transition-transform ${expanded ? 'rotate-180' : ''}`}>▾</span>
        </span>
        <span className="w-full text-[12px] text-faint num">{fmtDateTime(l.ts)}</span>
      </button>
      {expanded && (
        <div className="mt-3 pt-3 border-t border-line/60 grid sm:grid-cols-3 gap-2 text-[13px] text-mut">
        ...
        <div className="max-h-40 overflow-y-auto space-y-1 pr-1">
          {list.map((m) => (
            ...
            >
              ...
              <span className="ml-auto text-[12px] text-mut num">보유 {fmtNum(m.balances[c])}{CURRENCY_UNIT[c]}</span>
            </button>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-3">
